...
- **頭是刻意畫得很大的**（使用者的原話是「大頭狗開車」）。頭大到會互相蓋住，
  ...
  `BABY_HEAD_SCALE` 都會自動跟著變。瓶頸是駕駛到後座那 ~20 px。
  ⚠️⚠️ **嫌車大就只調 `HEAD_GAP_RATIO`**（車身尺寸跟它成正比），不要去動 `SEATS`
  或頭的大小。0.72 ＝ 兩顆頭剛好擦邊 ＝ 2.45 倍 ＝ 367 CSS px；現在是
  **0.55 ＝ ~1.9 倍 ＝ ~281 CSS px**（2026-09-03 使用者：「車子太大了」），
  代價是相鄰兩顆頭**輕微重疊** —— 完全不重疊與車不那麼大只能二選一，
