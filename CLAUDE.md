...
- **清單那一半併進既有的 `GET /api/notifications`**，不另外開一支。回應的 `items`
  ...
  ⚠️ 每一列左邊那一格：**留言那種放照片縮圖，上傳那種放那個人的頭像**
  （`actor_avatar`，兩句 SQL 各多帶一個 `avatar_key` —— D1 算的是讀了幾列不是幾欄，
  不多花額度；前端型別是**選填的**，邊快取裡躺著舊回應時退回「名字首字＋顏色」）。
  ⚠️⚠️ 上傳那種**曾經畫成一塊他的軌跡顏色的方塊**（2026-09-04 使用者：
  「通知 出現紅色框框 這應該是顯示該角色的頭像?」）—— 顏色在這個站只講一件事
  （地圖上那條線是誰的），拿它當通知圖示就是一塊沒來由的色塊。
...
- ⚠️⚠️ **「這段範圍沒有軌跡」是一句斷言，讀失敗不可以講成那樣**（2026-09-03
  使用者：「進去足跡選日期 有時候會突然顯示沒有足跡, 要重新選日期才會出現」）。
  兩個各自獨立的無聲失敗，都修了：
  ① `fetchTracks()` 把 `!res.ok` 與網路錯誤**吞成空陣列**，跟「真的沒有軌跡」
  分不出來 —— 現在**失敗回 `null`**，`/map` 收到 null 就**保留手上那份**
  並顯示紅字「軌跡讀取失敗，請重新選一次日期」（`tracksFailed`）。
  ② `loadTracks` 有**四個呼叫端**（換日期、換成員篩選、同步完、貼路完），
  其中三個掛不上 effect 的 cleanup，於是舊的回應會蓋掉新的 —— 現在一支
  遞增的 `tracksSeq` ref，`seq !== tracksSeq.current` 就整包丟掉。
  ⚠️ 這也是為什麼症狀是**時好時壞、重選一次日期就好**：重選＝再打一次，
  這一次剛好沒撞到。**看到「有時候會…，重做一次就好」先往這兩個方向查。**
...
- 圖在 `apps/frontend/public/`：`car-solo.webp`（一個人，450×289 device px）與
  `car-family.webp`（合體，585×273）。⚠️ 使用者原圖上那三顆綠點**在打包前就修掉了**，
  那是給人看座位用的參考記號，不是圖的一部分。
- ⚠️⚠️ **兩張是不同的畫，長寬比、座位、影子全都不一樣** —— 所以幾何寫成
  `car.ts` 裡那張逐張的 `SPRITES` 表（`src`／`w`／`h`／`seats`／`shadow`），
  對外只有 `carSize(sprite)`／`seatsOf(sprite)`／`seatOffset(sprite, seat, flip, t)`。
  **不要再回去寫共用的 `CAR_W`／`SEATS`** —— 2026-09-03 換合體那張圖時就是被那個
  共用常數擋住的：新的插畫後面拖著一台娃娃車，寬高比跟 solo 差很多。
  `seats` 存的是**正規化座標**（0–1，相對於那張圖自己）不是像素 ——
  換插畫只要重新量那三個小數。
- ⚠️⚠️ **座位標的是「脖子上緣」＝下巴要落在哪裡，不是頭的中心**
...
- ⚠️ **只要是合體軌跡，寶寶就一定在**（使用者拍板）—— 他不是一個成員，
  不參與人數與排序，`putSeat('rear', …)` 是無條件畫的。沒設頭像就畫小外星人。
  ⚠️ 座位名稱 `rear` 是**「這台車上的位置」不是「後座」** —— 2026-09-03 換的那張
  合體插畫裡，寶寶坐的是**車尾拖著的那台娃娃車**，不是車內的兒童座椅。
  語意（一定有他、最後才畫）完全沒變，只有座標換了。
- **一個人的時候用 `car-solo`，兩個人以上一律 `car-family`**（寶寶本來就在，
...
- **頭是刻意畫得很大的**（使用者的原話是「大頭狗開車」）。頭大到會互相蓋住，
  ...
  （2026-09-03 使用者拍板）。**唯一的解是把脖子的間距撐開**，也就是合體時整台車
  連座位一起放大（`CONVOY_CAR_SCALE`，一個人那台維持 1）。
  ⚠️ 那個倍數是**從 `seatsOf('family')` 現算的不是寫死的**（`HEAD_GAP_RATIO ×
  兩顆頭寬的平均 ÷ 兩個脖子的間距`，取最大的那一段，夾在 ≥1）—— 換插畫、
  調 `SEAT_HEAD_SCALE`／`BABY_HEAD_SCALE` 都會自動跟著變。
  ⚠️⚠️ **現在算出來是 ~1.10**（合體車 195 → ~214 CSS px），瓶頸永遠是
  副駕→駕駛那一段（間距 ~42 CSS px；駕駛→拖車上的寶寶隔了 ~75 px 一直很鬆）。
  換成「寶寶坐拖車」那張插畫時它一度掉回 1（舊圖是 ~44 ＋ ~20 px，非放大 2.45 倍
  不可），**2026-09-03 使用者說「合體的頭像可以再放大一點」，`SEAT_HEAD_SCALE`
  0.62 → 0.72，它就自己咬回來了** —— 那正是這段算式存在的意義：
  頭要多大由使用者決定，車跟著長，**頭永遠不離開脖子**。
  ⚠️ 副駕與駕駛那兩顆現在**重疊 ~25 px**（間距 ~46 vs 兩顆頭 ~71），
  比例跟使用者在舊插畫上已經接受的那一段一樣（`HEAD_GAP_RATIO` 沒動）。
  嫌車大就調 `HEAD_GAP_RATIO`（0.55，車身尺寸跟它成正比），不要去動座位或頭的大小。
  ⚠️ 車頂那一排（4 人以上）的 `HEAD_STEP` **刻意不共用這個比例**：那一排沒有脖子
  要貼、也沒有尺寸壓力，跟著疊起來只是白白變醜。
  ⚠️⚠️ **只放大車，頭不跟著放大** —— 頭一起放大等於原地踏步，遮擋一模一樣。
  ⚠️ 座位的位移（`seatOffset`）也要乘 `carScale`，車頂那一排的高度也是；
  但頭自己那張貼圖的透明邊 `sink` **不乘** —— 那跟車多大無關。
  ⚠️ 車的貼圖烤成 **`CAR_PIXEL_RATIO = 3`**（`BOB` 6 ／ `GROUND` 15 是 device px），
  **要改車的顯示大小請動 `CAR_PIXEL_RATIO`，不要動 `SPRITES` 裡的 `w`／`h`** ——
  後者是那張插畫自己的長寬比，改了畫面會被拉扁。
  ⚠️ 兩張車**現在不一樣大**（solo 150 ／ family 195 CSS px 寬），合體與散開的
  那一瞬間車會跳一階大小 —— family 那張後面多拖一台娃娃車，硬要一樣寬的話
  車身本體就縮水了。`w` 是照「車身的紅色部分跟 solo 對齊」定出來的。
  ...
  ⚠️ 合體那幾顆是 `SEAT_HEAD_SCALE`（0.72）縮過的，**要調合體的頭改這一個**
  （它一過 ~0.657 就會把車撐開，見 `CONVOY_CAR_SCALE`）；一個人那顆固定是 1。
  ...
  代價就是他浮在脖子上面。遮擋現在由插畫本身的間距＋畫的順序解決，
...
- ⚠️⚠️ **播放中的跟拍，使用者自己在動鏡頭時整段讓開**（2026-09-03 使用者：
  「動畫在跑的時候 以車子為中心點, 過程中可以放大或縮小地圖」）。
  跟拍是每一幀一句 `map.setCenter()`，而 maplibre 的 `jumpTo()` **開頭就是
  `this.stop()` → `handlers.stop(false)`** —— 那一句會把**正在進行中的手勢**
  整個中止掉，於是滾輪縮放／捏合／拖曳每一幀都被砍斷，畫面上就是「動不了」。
  作法是兩個 ref：`camDragRef`（手指還按著）與 `camHoldRef`（放手後再讓
  `CAMERA_HOLD_MS` = 700ms，等慣性補間滑完），兩個都不成立才 `setCenter`。
  ⚠️ **只擋 `setCenter`，從頭到尾不碰 zoom** —— 放開手之後下一幀鏡頭自己接回車上，
  而縮放比例是使用者剛剛調好的那個。
  ⚠️⚠️ **一定要聽 DOM 事件（`wheel`／`pointerdown`／`pointerup`／`pointercancel`／
  `dblclick`），不可以聽 maplibre 的 `movestart`／`moveend`** —— 跟拍那句
  `setCenter` 自己就會同步發一輪 movestart／move／moveend，而且**沒有
  `originalEvent`**，分不出是誰發的，聽它等於自己把自己判定成「使用者在動鏡頭」。
  ⚠️ 監聽掛在 `map.getCanvasContainer()` 不是 canvas。
- 車身是固定的紅色插畫，輪子也不轉了（那是畫上去的），改成整台車輕輕上下浮動
  （`carBob`）＋一片靜止的地面陰影。⚠️ 那片影子的位置與寬度也是**逐張存的**
  （`SPRITES[…].shadow`，family 的重心偏左才不會畫在拖車底下），
  而且**要跟著 `flip` 換邊** —— 它不在正中央，鏡射時不換就會浮到車外。
...
- ⚠️⚠️ **朝向與軌跡顏色 2026-09-04 從帳號牌整批搬進 `/admin`**（使用者要求
  ...
  **兩格都從帳號牌搬走**）。現在：
  - 「足跡地圖：車上的座位」那一格（`AdminSection id="car"`）端出**這台車上
    那三顆頭**：駕駛座（站長，固定）、**副駕駛（`AppSetting.seat_passenger_uid`
    指到的媽咪）**、後座的寶寶，每一顆都是頭像＋朝向＋**軌跡顏色**
    （寶寶沒有顏色 —— 他沒有帳號也沒有自己的軌跡）。座位、朝向與顏色本來就是
    同一件事（誰坐哪、臉朝哪、線是什麼色），分在兩頁等於要對照著調。
  ...
  - **其他人的頭像／朝向／顏色在白名單那一格**，每一列的「▸ 頭像與顏色」底下
    ...
    ⚠️⚠️ 顏色**一度只長在白名單那一列裡**，藏在那顆「▸ 頭像與顏色」小連結底下
    （2026-09-04 使用者：「後台沒看到軌跡顏色? 你放哪?」）—— 那一列右邊已經
    塞著相簿數、上傳數、最後上線、明細開關，再多一顆文字連結沒有人找得到。
    最常改的兩個人（自己、另一個站長）現在在座位那一格直接看得到。
  - ⚠️ **`AvatarPicker` 底下那段固定的說明由 `notes` 這顆選填 prop 控制**
    （預設 `true`）。同一頁擺好幾顆時一律 `notes={false}` —— 那段字是固定的，
    一顆一份等於同一句話連印三遍，把三顆頭推得老遠（車上那一格就是）。
    關掉的時候**那些話必須由呼叫端在整格開頭講一次**，不是省略掉。
    ⚠️ 只關這一段：挑完檔案才冒出來的那兩句（動圖不裁邊、沒偵測到透明背景）
    與錯誤訊息是**針對剛剛那張圖的回饋**，照樣要印。
    帳號牌那顆也 `notes={false}`（2026-09-04 使用者要求）—— 那張卡是每一頁都
    按得到的東西，不是設定頁。
...
- **手機（≤768px）那條時間軸刻意縮小**（`album.module.css` 最後那段 media query，
  2026-09-11 使用者：「在手機版本觀看時, 那個時間軸非常擋畫面」）。縮小之後只剩
  一排小點、月份的字整個藏起來（`.timelineNodeText { display: none }`），
  所以**點得到但看不出點的是哪個月** —— 下一條那個長按拖曳就是它的配套。
  ⚠️⚠️ **收著這條的寬度上限是「不可以碰到照片」**（2026-09-11 使用者：「時間軸
  可以再放大一點點, 不要蓋到照片格子就好」）。`.container` 兩側是 2rem＝32px 的
  padding，所以 `right` ＋這條的寬度必須留在 32px 以內；現在是 3 ＋ 22 ＝ 25px。
  **要更看得清楚是下面那個長按展開的事，不是把這條加寬。**
- **長按 0.2 秒可以拖著選月份**（`app/album/TimelineRail.tsx`，2026-09-11 使用者：
  「長按0.5秒的時候 會展開完整時間 手指上下滑動可選正確月份 放開即縮回去並且
  跳轉到該月份照片」）。按住整條就展開成完整的月份清單，上下滑動挑一個，
  放開就收回去並跳過去。
  ⚠️ `HOLD_MS` 當天從 500 調成 **200**（使用者：「按住時間改0.2秒」）——
  這條軌上沒有第二種長按手勢要區分，等半秒在手機上感覺像沒反應。
  `HOLD_MS` 越短底下那條 `SLOP_PX` 就越要緊，兩個數字是一組的。
  ⚠️⚠️ **展開的那一份刻意做大到會蓋住照片**（使用者：「展開的時間 可以放更大
  (蓋到照片都沒關係) 方便看到選到哪個月份」）—— 它只活在手指按著的那幾秒，
  而拇指就在最右緣，字級小的話根本看不出選到哪。**跟收著那條的規矩剛好相反，
  不要把兩邊的尺寸綁在一起。**
  - ⚠️⚠️ **那條軌是獨立元件，不可以搬回 `page.tsx`。** 拖曳中每換一個月就是一次
    `setState`，寫在相簿頁裡等於每一下都重畫整片格線（幾百張卡片）。
    `groups`／`onJump` 存在 ref 裡，所以監聽器那支效果相依是 `[]`。
  - ⚠️ 觸控監聽器一律原生 `{ passive: false }`（見「手勢」）。`touchmove` 裡除了
    `preventDefault()` 還要 **`stopPropagation()`** —— 父層掛在 window 上那支
    捲動處理器每一下都會掃過整片卡片的 rect，拖曳中用不到它。
  - ⚠️⚠️ **還沒滿 `HOLD_MS` 手指就移動超過 8px 就整個讓開**（`SLOP_PX`）。不讓的話
    這條貼著右緣的細軌會變成一塊捲不動的死區 —— 月份多的時候軌道自己也要捲。
  - ⚠️ `touchend` 要 `preventDefault()` 擋掉瀏覽器補發的 click，不然節點本來那個
    onClick 會再跳一次（跳到手指底下那一格，而且是平滑捲動）。
  - ⚠️ 計時器到點時先 `flushSync` 把展開的樣式畫上去**才**量位置 —— 展開前後
    每一格的高度差很多，拿收著的 rect 挑會挑錯。挑法是「手指高度按比例設
    `scrollTop`，再選離手指最近的那一格」：清單長過軌道時挑中的那格才會一直
    停在手指旁邊，短清單也對得準。
  - 拖曳選完那一跳是 **`instant`（`handleScrollToTimelineIndex` 的第二個參數）**：
    瞬間不是 smooth，而且把那個月的第一張放在**畫面上緣**不是置中 ——
    選的是「那個月」，置中的話上半個畫面還是上一個月。輕點節點照舊是平滑捲到中間。
  - ⚠️ 展開時月份的字排在點的**左邊**（`flex-direction: row-reverse`）：拇指按在
    最右緣，字放右邊會被手指整個蓋住。挑中的那格**只換顏色不換尺寸**，
    換尺寸的話每挑一格整份清單就跳一下。
  - ⚠️ 展開那幾條 CSS 一律寫成**兩層以上的選擇器**（`.timelineScrubbing .timelineNode…`），
    特異性高過手機那段 media query 的單一 class，**不必跟它比順序**
    （那段照舊必須留在 `album.module.css` 最後）。挑中那格要三層才壓得過
    手機那條 `.timelineNode:hover`。
...
- ⚠️⚠️ **讀相簿的那支效果相依只有 `[id, isAdmin]`，不可以放 `searchParams`**
  ...
  放進相依就是**每關一次燈箱整本重抓一次**。
- **`?photo=<id>` 深連結靠 `deepLinkKey`（`${id}:${photo}`）決定開不開**，不是
  「第一次載入才看」。從右上角「★ 精選」點進來時人可能已經在 `/album` 上
  （元件不重掛），只看第一次的話第二次點什麼都不會發生。找不到那張時**不記 key**
  （清單可能還沒載完）。
...
2026-09-03 加的。`/admin`「相片的像素比對」那一格（`app/admin/PhashCard.tsx`，
`AdminSection` id 是 `phash`），使用者的原話是「有些重複照片但特徵碼不同」。

- ⚠️ **`Photo.phash` 那一欄早就在**（`schema.sql` ＋ `idx_photo_phash`），`POST /api/upload`
  也一直讀著 `formData.get('phash')` 並綁進 INSERT —— **只是前端從來沒有送過**，
  所以存量每一列都是 NULL。**不需要 migration**，缺的一直只有生產者。
- **為什麼 `file_hash` 不夠**：它比的是位元組，而重新編碼過的同一張照片必定對不上
  —— Google 相簿匯入拿到的是 **Google 自己轉過的檔**，換一台電腦／瀏覽器重傳產生的
  800px 縮圖也不一樣。這一格改比**畫面本身**。
- **演算法是 dHash（64 bit）＋ 漢明距離**（`lib/phash.ts`）。縮成 **9×8 灰階、
  刻意不管長寬比** —— 同一張照片的 800px 與 400px 版本因此雜湊完全一樣。
  ⚠️ 不用 aHash（太吃亮度）也不用 pHash／DCT（兩倍的程式碼解一個我們沒有的問題）。
  ⚠️ 進 canvas 前**要先填白**：透明的 PNG／WebP 像素會被讀成黑色，跟它的 JPEG 版對不上。
  ⚠️ `dhashFromBlob` **絕不往外丟例外** —— 算不出來只是「這張比不出重複」。
- ⚠️⚠️ **雜湊在瀏覽器算，不在 Worker**（同旋轉縮圖、同 GPX）：Worker 沒有影像解碼器，
  也沒有 10ms CPU 以外的預算。**不要再往後端搬。**
  ⚠️ 一定要 `fetch` 位元組再走 objectURL／`createImageBitmap`，**不能把跨來源網址直接
  塞給 `<img>`** —— canvas 會被汙染，`getImageData` 當場丟例外。
  （`/api/photos/view/*` 有回 `Access-Control-Allow-Origin: *`，所以 fetch 這條走得通。）
- **成本是一次性的**：掃描時每張照片一次 Workers 請求 ＋ 一次 R2 class B（抓 400px 縮圖），
  一千張約吃掉每日 Workers 額度的 1%。算完**寫回 `Photo.phash`**，所以第二次按只會處理
  新增的那幾張；而新上傳的照片在 `uploadPhoto()` 裡自己就帶著 phash 上來
  （⚠️ 算的是**同一顆 400px 縮圖**，兩邊算出來一定一樣，不然存量與新照片會比不到一起）。
- **兩支路由合在 `/api/admin/phash`**（認 `canManageOthers` —— 回應裡是全站每一張照片）：
  `GET`（`cursor`／`limit`，一頁 500，第一頁順便回整張 `Album` 小表與 `total`）與
  `POST {hashes:[{id,phash}]}`（一趟最多 200 筆，走 `env.DB.batch`）。
  ⚠️ 兩支都**不可以包 `withEdgeCache`**，回 `Cache-Control: no-store`。
  ⚠️ 寫入端要驗 `^[0-9a-f]{16}$` 並去重，別人拿 token 直接打得到這一支。
- ⚠️ 相簿名字**不是用 JOIN 撈的**（同 drive-pending 的規矩）：`Album` 是幾十列的小表，
  第一頁整張回去在瀏覽器裡對。SELECT 多帶幾欄不多花讀取額度（D1 算的是讀了幾列不是幾欄），
  ⚠️ 但 **`url` 那一欄一定要帶** —— `photoThumbSrc()` 的逐級退回吃它，Google 同步進來的
  舊照片連 `thumb_url` 都是 NULL，漏了它那幾張永遠算不出雜湊、每次掃描都白花一次請求。
- **掃描是前端的迴圈**（同「比對全部相簿」、影片回讀、動態照片補掃、R2 掃描）：
  清單一頁 500 列推游標，算雜湊時 4 條併行、**每 200 張就寫回一次**
  —— ⚠️ 不要全部算完才寫，中途關掉分頁的話已經花掉的請求就白花了。
  讀不到縮圖的那幾張**刻意不寫 D1**（留在 NULL），畫面上講「再按一次會重試」。
- **分組在 `useMemo` 裡**，相依只有 `(items, threshold)` —— ⚠️ **換門檻不重抓也不重算**，
  雜湊都在手上了。先照「完全相同的雜湊」收桶（絕大多數的重複都落在這裡），
  再拿**不重複的那幾個雜湊**兩兩比距離、union-find 併起來（比幾千張兩兩比快得多）。
  門檻三段：一模一樣（0）／幾乎一樣（≤4，預設）／有點像（≤8）。
- ⚠️⚠️ **整片同色的畫面要整個排除**（`isFlatPhash`，全 0 或全 f）：全黑的影片封面、
  空白的掃描件雜湊都一樣，那不是「長得一樣」的證據而是「沒有東西可以比」——
  混進去會生出一大組假的重複。
- ⚠️ **連拍會被圈在一起，那不是 bug**：像素比對答的是「看起來一樣」不是「同一個檔」。
  這正是下一條存在的理由，而且那句話要寫在畫面上。
- ⚠️⚠️ **絕不自動刪、也不整組刪**（使用者拍板「逐張確認才刪」）。每一列各一顆刪除鈕，
  `window.confirm` 要把代價寫明：**連同它的標籤、留言、Story、手動修過的座標與時間一起沒，
  Drive 上那一份會排進 `trash/`**。哪一列該留只有人判斷得了（舊的那列往往帶著留言），
  同「站上多餘的列只列出來不刪」那條規矩。刪除走既有的 `DELETE /api/photos/:id`
  （`canTouchPhoto` 對 `canManageOthers` 短路），**不另做一支**。
- ⚠️ 刪完**不重抓**（同「不開放」那顆快速鎖）—— 重抓一次要好幾秒而且捲軸會跳，
  使用者得重新找剛剛看到哪一組。
- ⚠️⚠️ **但也不可以把那一列從 `items` 裡拿掉**（2026-09-03 使用者回報：
  「明明比對 可能有四五張一樣, 但點選刪除後 該項目就不見了, 不能繼續刪」）。
  分組是一支 `useMemo`，`items` 一變它就當場**整個重新分組** —— 一組只剩一張時
  `photos.length < 2` 那道守衛會把**整組丟掉**（union-find 靠某個雜湊串起來的那幾組
  還會散開），於是刪掉第一張，剩下那四張連同整組一起從畫面上消失。
  作法是另一份 `deletedIds`（Set），刪掉的**留在原地劃掉**（`.dupGone`，
  灰階縮圖＋刪除線＋「已刪除」取代那顆按鈕），分組完全不動。
  標題寫「（已刪 n 張）」、`dupCount` 只算還活著的那幾張。
  重新分組要等使用者自己按「重新比對」（`run()` 開頭清掉 `deletedIds`）。
- ⚠️ **「重新比對」隨時可以再按**，而且要在畫面上把這件事講出來 —— 使用者原話是
  「你又沒辦法重新比對」。功能其實一直在（按鈕在有結果之後就叫這個名字），
  但沒有一句話說得出「以後又傳到同一張、特徵碼不同的照片時，回來按它就會出現」。
  第二次按很便宜：算過的照片雜湊在 D1 裡、新上傳的自己就帶著上來，
  所以那一趟只有列表分頁的請求，不會再抓任何一顆縮圖。
- ⚠️ 不開放的照片在這裡照 `restrictedBlur` 糊掉、點一下掀開（共用 `restrictedReveal`）。
- ⚠️ 那一列的「看照片 ↗」是 **`/album?id=<相簿>&photo=<id>`**，不是 `/album/<相簿>`
  （靜態匯出沒有 `[id]` 那一層，見「補傳清單」）。
