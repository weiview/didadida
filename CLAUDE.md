...
- 圖在 `apps/frontend/public/`：`car-solo.webp`（一個人）與 `car-family.webp`
  （合體）。⚠️ **綠點在打包前就修掉了**，那是給人看座位用的參考記號，不是圖的一部分。
...
- **頭是刻意畫得很大的**（使用者的原話是「大頭狗開車」）。頭大到會互相蓋住，
  ...
  現在是車 180 CSS px 寬、一顆頭 116 —— 2026-09-02 從「車 260／頭 88」調過來的
  （使用者：「車子太大了」）。⚠️ 縮車連帶要調 `BABY_LIFT`（後座到駕駛只剩 ~24px，
  不抬高整顆會被擋掉）與車頂那一列的 `HEAD_STEP`／`HEAD_CROWD_SCALE`。
...
- 車身現在是固定的紅色插畫，**身分改由頭外面那一圈描邊（軌跡顏色）表示**。
  輪子也不轉了（那是畫上去的），改成整台車輕輕上下浮動（`carBob`）＋一片靜止的地面陰影。
