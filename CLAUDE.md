...
- **趟層**（`buildTrips` → `buildJointTrips`）：把動畫用的那份軌跡按 `day_key#seg` 切成一趟一趟，
  等距重取樣成 25m 點列（`TRIP_SAMPLE_M`），逐對算「A 有幾成的樣本落在 B 那條線 50m 內
  **且時間差 ≤5 分鐘**」。**分母取兩人裡較短的那個**（實作＝兩個方向取大的）。
  超過門檻就把兩趟真正重疊的那段時間標成同遊區間。
  沒有時間容差的話，每天同一條路上下班會被判成天天一起出遊。
