...
- **頭是刻意畫得很大的**（使用者的原話是「大頭狗開車」）。頭大到會互相蓋住，
  ...
  ⚠️⚠️ **以現在這張插畫算出來是 1，也就是不放大**。舊那張圖三個脖子橫向只隔
  ~44 ＋ ~20 CSS px（而一顆座位頭就 ~72 px），非放大到 2.45 倍不可；
  2026-09-03 換成「寶寶坐拖車」那張之後，脖子的間距變成 **~42 ＋ ~75 CSS px**，
  瓶頸那一段（駕駛→寶寶）當場鬆掉。**這一段留著是給下一次換插畫用的保險絲**，
  不是死碼 —— 哪天又換一張擠不下的圖，它會自己把車撐開。
  ⚠️ 副駕與駕駛那兩顆現在**輕微重疊 ~19 px**（間距 42 vs 兩顆頭 ~61）——
  跟使用者在舊插畫上已經接受的那一段一樣多，換來的是車只有 195 CSS px。
