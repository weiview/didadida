...
// 地圖上一律只有一台車（MOVER_EMOJI）：依速度猜出來的交通工具本來就常常猜錯
// （機車與汽車在速度上根本分不開），畫成一堆不同的車輛圖示等於把猜測當事實展示。
// 那台卡通小車不宣稱任何事，就只是「這個東西正在移動」。
//
// 這裡的 emoji 只給 HTML（播放列）用。地圖上那台是 car.ts 自己畫的動畫圖示 ——
// emoji 每台電腦長得不一樣而且不會動，當不了動畫的主角。

import type { Vehicle } from './api';

/** 播放列上的移動圖示。地圖上那台車的替身，尺寸小到只需要一個 emoji */
